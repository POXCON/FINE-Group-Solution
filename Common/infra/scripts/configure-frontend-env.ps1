...
.SYNOPSIS
  ...
  ポータル(Common) 用 frontend/.env.production を生成する。
  フロント本番ビルドの前に実行する。

  ポータルと Invoice-Search は同一の Cognito User Pool を共有し（SSO）、
  統合 CloudFront 同一オリジン配信のため API は /api/* 経由（相対）。
...
# API は統合 CloudFront 同一オリジンの /api/* 経由（VITE_API_BASE_URL は未設定=相対）。
