...
## 3. 初回起動
1. **再起動の要否**:
   - インストーラが **「再起動が必要」と表示したら従う**。
   - **WSL2・仮想化（Virtual Machine Platform/WSL機能）が既に有効**なら、それらの反映目的の**再起動は不要**。
   - ただし Docker はユーザーを `docker-users` グループに追加するため、**一度サインアウト→サインインが必要**な場合あり（しないと権限エラーが出ることがある）。表示が無ければそのまま次へ進み、起動でエラーが出たらサインアウト/サインイン（それでも不可なら再起動）。
